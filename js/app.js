...
convertButton.addEventListener("click", function () {
  ...
  const selectedLabRadio = document.querySelector(
    'input[name="labOptions"]:checked'
  );
  const selectedStudyRadio = document.querySelector(
    'input[name="studyOption"]:checked'
  );
  const selectedsampleTypeRadio = document.querySelector(
    'input[name="sampleTypeOption"]:checked'
  );

  let selection =
    selectedLabRadio.value +
    selectedStudyRadio.value +
    selectedsampleTypeRadio.value;

  const errorCheck = squenceErrorCheck(preExcelDB, preExcel);
...
const squenceErrorCheck = (preExcelDB, hotInstance = preExcel) => {
  let sequenceArray = [];
  preExcelDB = preExcelDB.filter((row) =>
    row.some((cell) => cell !== "" && cell != null)
  );
  preExcelDB.map((column, index) => sequenceArray.push([column[8], index]));
  let failed = false;

  prevIndex = 0;

  for (let currIndex = 1; currIndex < sequenceArray.length; currIndex++) {
    let prevIndexNumber;
    let currIndexNumber;
    const prevRangeBool = sequenceArray[prevIndex][0].includes("-");
    const currRangeBool = sequenceArray[currIndex][0].includes("-");
    const prevRange = sequenceArray[prevIndex][0]
    const currRange = sequenceArray[currIndex][0]

    if (prevRangeBool) {
      prevIndexNumber = Number(prevRange.split("-")[1]);
    } else {
      prevIndexNumber = Number(prevRange);
    }

    if (currRangeBool) {
      currIndexNumber = Number(currRange.split("-")[0]);
    } else {
      currIndexNumber = Number(currRange);
    }

    if (prevIndexNumber == 81 && currIndexNumber == 1) {
    } else if (currIndexNumber != prevIndexNumber + 1) {
      hotInstance.setCellMeta(currIndex, 8, 'className', 'highlight-cell');
      hotInstance.render();
      return true
    }
    prevIndex++
